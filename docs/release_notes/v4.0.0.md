...
This release supersedes `v3.1.0` and `v3.2.0`, neither of which was ever actually published to crates.io: `v3.1.0`'s crates-publish run was rejected by the workflow's own version-consistency check because that commit's `Cargo.toml` had already been bumped to `3.2.0` by an unrelated dependency-upgrade PR, and `v3.2.0` was never taken out of draft. The last version actually available on crates.io remains `3.0.0`. `v4.0.0` reflects that: it's a major version bump both because of the breaking `query()` change above, and because it is the true, sole successor to `3.0.0`.
