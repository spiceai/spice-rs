...
#[derive(Debug, Snafu)]
pub enum Error {
    Flight { source: crate::flight::Error },
}
...
impl SpiceClientConfig {
    ...
    pub async fn load_from_default() -> Result<SpiceClientConfig> {
...
impl SpiceClient {
    ...
    pub async fn query(&mut self, query: &str) -> Result<FlightRecordBatchStream> {
        self.flight.query(query).await.context(FlightSnafu)
    }
    ...
    /// # }
    /// ````
    pub async fn fire_query(&mut self, query: &str) -> Result<FlightRecordBatchStream> {
        self.firecache.query(query).await.context(FlightSnafu)
    }
    ...
    //     self.prices.get_supported_pairs().await.context(FlightSnafu)
    // }
    ...
    //     self.prices.get_prices(pairs).await.context(FlightSnafu)
    // }
